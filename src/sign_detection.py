...
# Import your functions here (training, localization, etc.)
from classification import training, getLabel

SIGNS = [
    "ONE WAY", "STOP", "TURN RIGHT", "TURN LEFT", 
    "NA", "NA", "ONE WAY", "NA", "NA"
]
...
class SignDetectionNode:
    def __init__(self):
        ...
        # Parameters
        self.min_size_components = rospy.get_param('~min_size_components', 300)
        self.similarity_threshold = rospy.get_param('~similarity_contour_with_circle', 0.65)
    ...
    def image_callback(self, msg):
        ...
        # Process the image
        coordinate, processed_image, sign_type, text = localization(
            frame, 
            self.min_size_components, 
        ...
        if coordinate is not None:
            cv2.rectangle(processed_image, coordinate[0], coordinate[1], (255, 255, 255), 1)

        cv2.imshow('Processed Image', processed_image)
        cv2.waitKey(1)  # Required to refresh OpenCV windows

        # Log or take actions based on the detected sign
        if sign_type > 0 and sign_type != self.current_sign:
            rospy.loginfo(f"Detected sign: {text}")
...
def main():
    try:
        ...
        rospy.loginfo("Shutting down sign detection node.")
    finally:
